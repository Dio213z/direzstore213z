...
function renderGames() {
  ...
  games.forEach(game => {
    ...
    if (game.id === 'redfinger') {
      card.onclick = () => window.open('https://direzstore.my.id/redfinger', '_blank');
    } else if (game.id === 'genshin') {
      card.onclick = () => window.open('https://direzstore.my.id/genshin-impact', '_blank');
    } else if (game.id === 'telegram') {
      card.onclick = () => window.open('https://direzstore.my.id/telestars', '_blank');
    } else if (game.id === 'roblox') {
      card.onclick = () => window.open('https://direz-store-robloxrobux.my.canva.site', '_blank');
    } else {
      card.onclick = () => showPrices(game.id, game.name);
    }
...
function showPrices(gameId, gameName) {
  ...
  if (priceTitle) priceTitle.textContent = `Pilih Paket ${gameName}`;
  ...
  if (prices[gameId]) {
    ...
    Object.entries(grouped).forEach(([category, items]) => {
      ...
      items.forEach((item) => {
        ...
        if (!item.habis) {
          box.onclick = (e) => selectPrice(item, e);
        } else {
...
function selectPrice(item, event) {
  ...
  if (summaryGame) summaryGame.textContent = currentOrder.gameName;
  if (summaryPackage) summaryPackage.textContent = item.package;
  if (summaryTotal) summaryTotal.textContent = `Rp${item.price.toLocaleString('id-ID')}`;
...
function updateTotal() {
  ...
  if (qtyDisplay) qtyDisplay.textContent = currentOrder.quantity;
...
function cancelOrder() {
  ...
  if (priceSection) priceSection.classList.remove('active');
...
async function submitOrder(e) {
  ...
  if (window.dataSdk) {
    ...
    // Fallback jika SDK tidak tersedia
    showReceipt(orderNum, currentOrder.gameName, currentOrder.package, nickname, total, selectedPayment, gameId, serverId, email, redeemCode);
    cancelOrder();
  }
}
...
function showReceipt(orderNum, game, pkg, nickname, total, payment, gameId, serverId, email, redeemCode) {
  ...
  const modalRedeemCode = document.getElementById('modalRedeemCode');
  
  if (modalOrderNum) modalOrderNum.textContent = orderNum;
  if (modalGame) modalGame.textContent = game;
  if (modalPackage) modalPackage.textContent = pkg;
  if (modalGameId) modalGameId.textContent = gameId;
  if (modalNickname) modalNickname.textContent = nickname;
  if (modalPayment) modalPayment.textContent = payment.toUpperCase();
  if (modalTotal) modalTotal.textContent = `Rp${total.toLocaleString('id-ID')}`;
  if (modalRedeemCode) modalRedeemCode.textContent = redeemCode;
...
function openHistory(e) {
  ...
  const historyPhoneInput = document.getElementById('historyPhoneInput');
  ...
  if (historyFormSection) historyFormSection.style.display = 'block';
...
function searchHistory() {
  ...
  const historyFormSection = document.getElementById('historyFormSection');
  if (historyFormSection) historyFormSection.style.display = 'none';
  list.style.display = 'block';
}
